...
local
   ...
   fun {DefProc Tokens Rest}
        Rest1 Rest2 in
            Tokens = '{' | '$' | Rest1 'proc'( { IdentList Rest1 '}' | Rest2 }
                                               { Stat Rest2 'end' | Rest2 } )
   end
   ...
   fun {If Tokens Rest}
        try
            Rest1 Rest2 in
                'cond'( { Ident Tokens 'then' | Rest1 }
                        { Stat Rest1 'else' | Rest2 }
                        { Stat Rest2 'end' | Rest } )
        catch E then
            raise ifPE(E) end
        end
   ...
   fun {Case Tokens Rest}
        try
            Rest1 Rest2 Rest3 in
                pat_mat( { Ident Tokens 'of' | Rest1 }
                         { Pat Rest1 'then' | Rest2 }
                         { Stat Rest2 'else' | Rest3 }
                         { Stat Rest3 'end' | Rest } )
   ...
   fun {Application Tokens Rest}
        try
            Rest1 in
                proc_app( { Ident Tokens Rest1 }
                          { IdentList Rest1 '}' | Rest } )
        catch E then raise applicationPE(E) end
        end
   end
